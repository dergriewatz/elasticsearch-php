...
use Elasticsearch24\Common\Exceptions\RuntimeException;
use Elasticsearch24\ConnectionPool\AbstractConnectionPool;
use Elasticsearch24\ConnectionPool\Selectors\SelectorInterface;
use Elasticsearch24\ConnectionPool\StaticNoPingConnectionPool;
use Elasticsearch24\Connections\Connection;
use Elasticsearch24\Connections\ConnectionFactory;
use Elasticsearch24\Connections\ConnectionFactoryInterface;
use Elasticsearch24\Serializers\SerializerInterface;
use Elasticsearch24\ConnectionPool\Selectors;
use Elasticsearch24\Serializers\SmartSerializer;
...
class ClientBuilder
{
    ...
    /** @var string */
    private $connectionPool = '\Elasticsearch24\ConnectionPool\StaticNoPingConnectionPool';

    /** @var  string */
    private $serializer = '\Elasticsearch24\Serializers\SmartSerializer';

    /** @var  string */
    private $selector = '\Elasticsearch24\ConnectionPool\Selectors\RoundRobinSelector';
    ...
    /**
     * @param \Elasticsearch24\Connections\ConnectionFactoryInterface $connectionFactory
     * @return $this
     */
    ...
    /**
     * @param \Elasticsearch24\ConnectionPool\AbstractConnectionPool|string $connectionPool
     * @param array $args
    ...
    /**
     * @param \Elasticsearch24\Serializers\SerializerInterface|string $serializer
     * @throws \InvalidArgumentException
     * @return $this
     */
    ...
    /**
     * @param \Elasticsearch24\ConnectionPool\Selectors\SelectorInterface|string $selector
     * @throws \InvalidArgumentException
     * @return $this
     */
    ...
    public function build()
    {
        ...
        if (is_null($this->endpoint)) {
            ...
            $this->endpoint = function ($class) use ($transport, $serializer) {
                $fullPath = '\\Elasticsearch24\\Endpoints\\' . $class;
     ...
     *
     * @throws \InvalidArgumentException
     * @return \Elasticsearch24\Connections\Connection[]
     */
